...
import { parseImportPreview, type ImportPreviewState } from "@/lib/imports/preview";

const initialBatchState: ImportBatchActionState = { status: "idle", message: "" };
...
export function ImportWorkbench({ sampleCsvText }: { sampleCsvText: string }) {
  ...
  const [preview, setPreview] = useState<ImportPreviewState>(() =>
    parseImportPreview(sampleCsvText, "sample-template.csv"),
  );
  ...
  function updatePreview(nextCsvText: string, fileName: string) {
    startTransition(() => {
      ...
      setPreview(parseImportPreview(nextCsvText, fileName));
    });
  }
